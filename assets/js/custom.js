$(function () {
    ...
    if ($('.range_slider').length > 0) {
        const minPrice = window.shopData?.minPrice || 0;
        const maxPrice = window.shopData?.maxPrice || 10000;
        const currentMin = window.shopData?.currentMin || minPrice;
        const currentMax = window.shopData?.currentMax || maxPrice;

        $('.range_slider').alRangeSlider({
            range: { min: minPrice, max: maxPrice, step: 10 },
            initialSelectedValues: { from: currentMin, to: currentMax },
            grid: { minTicksStep: 1, marksStep: 5 },
            theme: "dark",
            onFinish: function (data) {
                // Determine min/max from data structure
                let min = data?.from;
                let max = data?.to;

                if (min === undefined && data?.selectedValues) {
                    min = data.selectedValues.from;
                    max = data.selectedValues.to;
                } else if (min === undefined && Array.isArray(data)) {
                    min = data[0];
                    max = data[1];
                }
                ...
                if (min !== undefined && max !== undefined) {
                    ...
                    console.error("Price filter error: Invalid data received", data);
                }
            }
        });
    }
